...
#### 1. 全站 UI/UX 核心準則 (Design System)

為了根除「字體偏小、配色過黑、層次不足」的問題，確立以下實作標準：

- **字體規範 (Typography Standards)**：
  - **字級標準**：建立 `--text-xs` (13px) 到 `--text-2xl` (28px) 的變數系統。**全站嚴禁使用低於 13px 的字體**。
  - **字重層次**：深色模式內文提昇至 `450-500` (Medium) 以增加可讀性；標題固定在 `600` (Semi-bold) 避免暈開。
  - **檔案式間距**：標籤與元數據增加 `0.08em` 字距，Hero 標題增加 `0.15em`，營造「電影檔案庫」儀式感。
  - **行高優化**：閱讀型長文字行高固定為 `1.75 - 1.85`；UI 指令文字縮緊至 `1.25`。
  - **技術數據等寬化**：年份、片長、座標、Admin 數據強制使用 Mono 字體 (IBM Plex Mono)。

- **色彩與海拔 (Color & Elevation)**：
  - **深炭灰取代死黑**：底層背景改為 `#121418` (帶微藍調深炭灰)。
  - **海拔系統**：透過明度差建立層次：`Base (#121418)` < `Surface (#1c1e22)` < `Raised (#24272c)`。
  - **文字對比度**：主文字使用 `#f0f2f5` (0.94 鮮明度)；次要文字提昇至 `0.65` 透明度。

#### 2. 放映廳 (Theater) 佈局重構

- **模組化導覽**：引入 Tab 分頁（概覽 / 策展 / 討論板），將目前的垂直長捲軸模組化。
- **展示升級**：
  - 實作 `MovieCarousel` (橫向轉盤) 處理「建議先看」與「共同片單」。
  - 海報比例建議改為 **2:3 (160x240)**，資訊疊加於海報底部。
  - 成員片單改為「海報牆」視覺，將繁雜 Meta data 移至 Hover 狀態。
- **交互解耦**：將建立片單、搜尋電影等表單移至 Modal 或側邊滑出層，保持主頁面純粹。

#### 3. 全局提亮計畫 (Site-wide Polishing)

- **Admin 後台**：圖表 Tooltip 與標籤統一使用 `var(--text-xs)`，套用 `--admin-surface` 提高明度。
- **Ticket / DNA**：調整卡片寬度，確保 13px 標籤能優雅排版，不影響視覺平衡。
- **Auth 表單**：改善輸入框提示文字大小與對比度，降低 Modal 壓迫感。
